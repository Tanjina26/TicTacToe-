...
function checkWin() {
	...
	//function for alerting when X or O win
...
function checkDraw() {
	...
	if ( !($('.tile').hasClass('box')) ) {
		alert("No winner yet. Try playing again!");
		clearBoard();
	}
}
...
function Oplay() {
	...
	//this is the function for when the computer, player O, plays tactically. 
...
//making sure that only an empty square can be selected.
$('.tile').on('click', function Xplay() {
	...
	if (playValid) {
		...
		alert("That square has been used. Please choose an empty square");
	}
	
})

//below is the reset button to start over at any time.
